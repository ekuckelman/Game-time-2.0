 var blocksArray;

class Blocks {
  constructor({w, h}) {
    this.width = w;
    this.height = h;
    this.color = 'white';
  }
  
  isActive(context, brickX, brickY) {
      context.beginPath();
      context.rect(brickX, brickY, this.width, this.height);
      context.fillStyle = 'blue';
      context.fill();
      context.closePath();
  } 

  createBlockArray(){
    blocksArray = [];
      for(let c = 0; c < 9; c++) {
        blocksArray[c] = [];
       for(let r = 0; r < 4; r++) {
        blocksArray[c][r] = {
          x:0,
          y:0,
          status: 'active',
        }
      }
    }
  }
   
  drawBlocks(context) { 
    for (var c = 0; c < 9; c++) {
      for (var r = 0; r < 4; r++) {
        if (blocksArray[c][r].status === 'active') {
          var brickX = (c * (this.width + 2)) + 10;
          var brickY = (r * (this.height + 2)) + 10;
          blocksArray[c][r].x = brickX;
          blocksArray[c][r].y = brickY;
          this.isActive(context, brickX, brickY)
        }
      }
    }
  }

  blockBallCollision(newBall,context){
    for (var c = 0; c < 9; c++) {
      for (var r = 0; r < 4; r++) {
        var eachBlock = blocksArray[c][r];
          if (eachBlock.status === 'active') {
            if (newBall.x > eachBlock.x && newBall.x < eachBlock.x + this.width && newBall.y - newBall.radius < eachBlock.y + this.height && newBall.y - newBall.radius < eachBlock.y + this.height) {
              newBall.dy = - newBall.dy;
              eachBlock.status = 'inactive';
            }
          }
      }
    }
  }
}

module.exports = Blocks;


